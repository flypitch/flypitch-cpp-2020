...
% We end this section by remarking that the ``smallness'' of $\B$ (or more precisely, the fact that $\B$ lives in the same universe of types out of which \lil{bSet 𝔹} is being built) is essential in making \lstinline{bSet 𝔹} a model of $\ZFC$.
...
\end{lstlisting}
That is, we send each \lil{c : ℙ_cohen} to all subsets satisfying the specification given by \lil{c}. This is clopen, hence regular.
\end{defn}
...
\paragraph{CCC and the \(\Delta\)-system lemma} To show that \(\B_{\mathsf{cohen}}\) has the CCC, we formalize and then apply a general result in transfinite combinatorics called the \emph{$\Delta$-system lemma}. Though only briefly mentioned in \cite{DBLP:conf/itp/HanD19}, this was one of the most involved parts of our formalization of Cohen forcing, as it required significant extensions to the treatment of set theory in \texttt{mathlib}. The details of the full argument are too technical to give here. Instead, we first give an elementary proof that \(\B_{\mathsf{cohen}}\) has the CCC, the key ideas of which are vastly generalized by the \(\Delta\)-system lemma (\Cref{lemma:delta-system-lemma:general}).
...
This style of argument can be refined to prove the far more general \(\Delta\)-system lemma; this is how it was implemented in our formalization. A family $(A_i)_i$ of sets is called a \textbf{$\Delta$-system} if there is a set $r$, called the \textbf{root} such that whenever $i \ne j$ we have $A_i \cap A_j = r$.

\begin{lemma}[\(\Delta\)-system lemma (Theorem 1.6, \cite{kunen2014set})]\label{lemma:delta-system-lemma:general}
  Let \(\kappa\) be an infinite cardinal and let \(\theta > \kappa\) be regular, such that \(\forall \alpha < \theta, \alpha^{<\kappa} < \theta\). For any family \(\mathcal{A} := (A_i)\) such that \(\#\mathcal{A} \geq \theta\) and for all \(i\), \(\#A_i < \kappa\), there is a subfamily of size \(\theta\) which forms a \(\Delta\)-system.
\end{lemma}

To apply this lemma to showing \(\B_{\mathsf{cohen}}\) has the CCC, we formalized the following results,  which required us to significantly extend the treatment of product topological spaces in \texttt{mathlib}. First, note that setting \(\kappa = \omega\) and \(\theta = \aleph_1\) in \Cref{lemma:delta-system-lemma:general} yields:
% First, we explain how we use a special case of the full \(\Delta\)-system lemma to show \(\B_{\mathsf{cohen}}\) has the CCC.
...
\begin{thm}\label{thm:product-ccc}
  ...
  \end{thm}
  \begin{proof}
  Let \(\{U_k\}\) be an uncountable family of pairwise disjoint opens of $\prod_{i\in I} X_i$. By shrinking $U_k$, we may assume $U_k$ is a basic open of the form $\prod_{i\in F_k} U_{k,i} \times \prod_{i \not\in F_k} X_i$ for some finite $F_k\subseteq I$ and $U_{k,i}\neq X_i$ open in $X_i$. The $(F_k)_k$ form a uncountable family of finite sets, so by \Cref{lemma:delta-system-lemma:simple} we know that there is an uncountable family $K$ of indices such that $(F_k)_{k\in K}$ forms a $\Delta$-system with root $J$. Taking the projections $\pi(U_k)$ onto $\prod_{i\in J}X_i$ for $k\in K$, we obtain an uncountable disjoint family of opens in $\prod_{i\in J}X_i$, a contradiction.
\end{proof}

\Cref{lemma:cohen-algebra-CCC} follows immediately from \Cref{thm:product-ccc} and the observation that $2^J$ has the CCC if $J$ is finite.

TODO(floris) brief remarks about why formalizing \Cref{lemma:delta-system-lemma:general} is impressive
...
We also want to formalize the consistency of our extension of \(\mathsf{ZFC}\) by definable function sybmols, so that $\CH$ is independent of the usual presentation of $\ZFC$ in the language \(\{\in\}\). Furthermore, while formulas implemented with de Bruijn indices enjoy pleasant theoretical properties for the purposes of a formalization, they are difficult to write and debug by hand. It should be possible with Lean's metaprogramming to write a custom parser from named variables.
...
Besides \(\mathsf{L}\), the consistency of $\mathsf{GCH}$ can also be shown by an iterated forcing argument. Our current implementation of forcing should extend without too much difficulty to iterated forcing with Boolean-valued models.  % There are also many generalizations of the consistency of $\neg \mathsf{CH}$.
