...
% After observing (1) that the Aczel encoding of $\mathsf{ZFC}$ as an inductive type is essentially a special case of the recursive \emph{name} construction from forcing (\Cref{sect:bset}),
...
% However, the user must be careful to choose the encoding so that properties they wish to reason about are accessible by structural induction, which is the most natural mode of reasoning in the proof assistant.
% After observing (1) that the Aczel-Werner encoding of $\mathsf{ZFC}$ as an inductive type is essentially a special case of the recursive \emph{name} construction from forcing (c.f. \ref{sect:boolean-semantics}),
% and (2) that the automatically-generated induction principle for that inductive type \emph{is} $\in$-induction,
% it is easy to see that this encoding can be modified to produce a Boolean-valued model of set theory where, again, $\in$-induction comes for free.
...
If we can construct two Boolean-valued models of $\ZFC$, one where $\CH$ holds, and one where $\CH$ fails, then we can apply the Boolean-valued soundness theorem to show that $\CH$ is independent from $\ZFC$.
...
This is precisely the task of forcing (\Cref{sect:forcing}).
...
We use custom domain-specific tactics and automation throughout our formalization, notably a tactic library for simulating natural deduction proofs inside a complete Boolean algebra \Cref{sect:metaprogramming}). This is another advantage of working in a proof assistant: the bookkeeping of Boolean truth-values, sometimes regarded as a tedious aspect of the Boolean-valued approach to forcing, can be hidden via automation.
...
Our main contribution is a formalization of collapse forcing and the unprovability of \(\neg \CH\), thereby providing the first formalization of the independence of \(\CH\) in a single theorem prover. For reasons we will see in \Cref{sect:forcing}, the forcing argument for \(\CH\) requires far more set theory and is significantly harder to formalize than the forcing argument for \(\neg \CH\). Moreover, we also elaborate on parts of the formalization---such as our exact formulation of the \(\ZFC\) axioms and the steps we have taken to ensure that we have truly proved \(\CH\) independent from \(\ZFC\)---which were omitted from \cite{DBLP:conf/itp/HanD19}. % The material in X and Y is entirely new. We unify and streamline the presentation of the forcing arguments in \Cref{sect:forcing}, added a discussion of bar, and expanded our discussion of baz.
...
We could also define an inductive type where the constructors \lil{rel} and \lil{apprel} were replaced by the single constructor
\begin{lstlisting}
...
Besides Boolean-valued semantics (\Cref{sect:boolean-semantics}), we also formalize ordinary first-order semantics, and our work includes a formalization of the completeness theorem using Henkin models.

\subsection{ZFC}
\label{subsect:fol:zfc}
% note(jesse, October 18 2019, 08:44 PM): compressed this section, which was a bit wordy
...
Notably, we specify ordered pairs by enforcing \lil{∀ x y z w, (x,y) = (z,w) ↔ x = z ∧ y = w} (the usual axiom of pairing is derivable from the existing axioms), and we extend the axiom of infinity with a specification of \lil{ω} as the least limit ordinal.

% %
% % eight axioms and a strong version of the axiom schema of collection.
...
% % % note: avoid contractions in academic papers
...
One can then define membership from equivalence and check that modulo extensional equivalence, \lstinline{pSet} is a model of \(\mathsf{ZFC}\).
...
\(\B\)-valued predicate interpreting the membership symbol \lil{∈}. We will encode this information by extending each \lil{⟨α,A⟩ : pSet} with an additional function \lil{B : α → 𝔹}, which has the effect of attaching a \emph{Boolean truth-value} to every member of \lil{⟨α,A⟩}:
\begin{lstlisting}
...
| mk (α : Type u) (A : α → bSet)
    (B : α → 𝔹) : bSet
\end{lstlisting}
The \lil{𝔹}-valued predicate \lil{B} expresses that \lil{A a ∈ ⟨α, A, B⟩} has truth value (at least) \lil{B i}.
...
In particular, if \(\mathbb{P}\) is the singleton poset, then a \(\mathbb{P}\)-name is merely a set of \(\mathbb{P}\)-names, in the same way that a term of type \lstinline{pSet} is a type-indexed collection of terms of type \lstinline{pSet}. Reversing this observation, we can replace \(\mathbb{P}\) with a complete Boolean algebra \(\mathbb{B}\) and generalize the definition of \lstinline{pSet.mk} with a third field, so that as in the case of \(\mathbb{P}\)-names, every element of a set is assigned an element (a ``Boolean truth-value'') of \(\mathbb{B}\), again giving us \lil{bSet 𝔹}. Thus, \lil{bSet 𝔹} should be thought of as the type of \lil{𝔹}-names
...
We remark that while standard treatments of Boolean-valued models of \(\ZFC\) mutually define equivalence and membership so that the axiom of extensionality follows definitionally (\cite{bell2011set}, \cite{hamkins2012well}), the induction principle given by the non-mutual definition is easier to work with in our formalization.
...
\begin{defn} \label{def-powerset}
  ...
  We can define the $\B$-valued set $\widetilde{\chi}$ as \lil{⟨α, A, χ⟩}.
...
\subparagraph*{Ordinals in \lil{bSet 𝔹}}
...
From the Boolean-valued perspective, forcing a sentence \(\Phi\) in the language of \(\mathsf{ZFC}\) means constructing some Boolean algebra \lil{𝔹} and a \lil{𝔹}-valued model \(M\) of \(\ZFC\) such that the truth value \(\Phi^{M}\) of \(\Phi\) is \(\top\).
We will always force over a type universe \lil{Type u}, and our Boolean-valued models of \(\mathsf{ZFC}\) are always of the form \lil{bSet 𝔹} for some \lil{𝔹 : Type u}.
...
The basic strategy of Cohen forcing is to choose \lil{𝔹} such that for every \(\nu : \aleph_2\), there is a canonical indicator function (a ``Cohen real'') \(\chi_{\nu} : \omega \to \mathbb{B}\).
...
\end{lstlisting}
That is, we send each \lil{c : ℙ_cohen} to all the subsets which satisfy the specification given by \lil{c}. This is a clopen set, hence regular.
\end{defn}
Crucially, the image of this embedding is a dense suborder of \(\mathbb{B}_{\mathsf{cohen}}\).
Recalling that $\leq$ in $\B$ is subset-inclusion, we see that this is essentially because the image of $\iota : \mathbb{P}_{\mathsf{cohen}} \to \B_{\mathsf{cohen}}$ \emph{is} the standard basis for the product topology.
Our chosen encoding of the Cohen poset also made it easier to perform this identification when formalizing this proof.

To show that \(\B_{\mathsf{cohen}}\) has the CCC, we use a powerful combinatorial argument called the \emph{$\Delta$-system lemma}. A family $(A_i)_i$ of sets is called a \textbf{$\Delta$-system} if there is a set $r$, called the \textbf{root} such that whenever $i \ne j$ we have $A_i \cap A_j = r$.
\begin{lemma}[$\Delta$-system lemma] \label{lemma:delta-system-lemma}
  Any uncountable family of finite sets has an uncountable subfamily forming a $\Delta$-system.
\end{lemma}
% TODO: maybe talk about technical implementation of proof? even though we didn't need the full lemma, this was definitely an impressive technical feat of our formalization of Cohen forcing, and would constitute new material.
We omit the proof, referring the interested reader to Kunen~\cite{kunen2014set} or the formalization.
We use this to show the following theorem. We say that a topological space has the countable chain condition if every family of pairwise disjoint open sets is countable.
...
\end{lemma}
\begin{proof}
  It suffices to show that the topological space $2^{\aleph_2 \times \mathbb{N}}$ has the countable chain condition, which follows immediately from \Cref{thm:product-ccc} and the observation that $2^J$ has the countable chain condition if $J$ is finite.
\end{proof}

\begin{defn}\label{def:principal-open}
...
To finish negating \(\mathsf{CH}\), it suffices to show that
\[\omega \prec \widecheck{\aleph_1} \prec \widecheck{\aleph_2}.\]
...
We will prove the latter claim; an identical argument can be used to show the former claim.
...
The formalization of Cohen forcing is actually much easier than the formalization of collapse forcing. In Cohen forcing, unlike collapse forcing, constructing the new function (and actually the entire forcing argument) required no density arguments at all. This is because in order to force \(\neg \CH\), we only had to ensure there was some infinite cardinality between \(\omega\) and \(\mathcal{P}(\omega)\), whereas to force \(\CH\), we had to determine the exact cardinality of \(\widecheck{\aleph_1}\) to be \(\aleph_1\). However, proving that \(\mathbb{B}_{\mathsf{cohen}}\) has the CCC is more delicate than just observing that a union of countable partial functions is again countable.
...
\begin{defn}\label{def-collapse-poset}
  We define \lstinline{ℙ_collapse} to be the poset of countable partial functions \(\aleph_1 \to \mathcal{P}(\omega)\).
...
\begin{remark}
  As an implementation detail, in the formalization we \emph{define} \lil{ℙ_collapse} to be the countable partial functions (in \lil{Type u}) between \lil{(ordinal.mk (aleph one) : pSet).type} and \lil{(powerset omega : pSet).type}, so that \\
  \lil{𝔹_collapse}-valued indicator functions on
  \lil{ordinal.mk (aleph one) : pSet).type × (powerset omega : pSet).type}
  are definitionally equal to \lil{𝔹_collapse}-valued indicator functions on the underlying types of \lil{check (ordinal.mk (aleph one))} and \lil{check (powerset omega)}.
\end{remark}
...
To finish demonstrating that \(\CH\) is true in \lil{bSet 𝔹_collapse}, it remains to check that \(\widecheck{\mathcal{P}(\omega)} = \mathcal{P}(\omega)\) and \(\widecheck{\aleph_1} = \aleph_1\).
...
First, the axiom of comprehension (\Cref{def-comprehension}) is applied to \(\mathcal{P}(\omega \times \omega)\) to produce (what \lil{bSet 𝔹} thinks is) the collection of all relations \(R\) on \(\omega\) such that there exists an ordinal \(\eta\) and an injection \(f : \eta \hookrightarrow \omega\) such that \(R\) is the image of \(\eta\)'s membership relation under \(f\).
...
It remains to show that \lil{𝔹_collapse} fulfills the assumptions of \Cref{lemma-function-reflect-suffices}.

\begin{lemma}\label{lemma-function-reflect}
  For every \lil{ y : pSet}, and every \lil{f : bSet 𝔹_collapse} such that \lil{bSet 𝔹_collapse} models that \lil{f} is a function from \(\omega\) to \(\widecheck{y}\),
  there exists a \lil{g : pSet} such that \lil{g} is a function from \(\omega\) to \lil{y} in \lil{pSet}, and \lil{bSet 𝔹_collapse} models that \(\widecheck{g} = f\).
\end{lemma}
...
\end{lstlisting}
  ...
  As with Cohen forcing, we will reflect a Boolean-valued \(\forall\)-\(\exists\) statement into the metatheory, and then use a combinatorial property of \lil{𝔹_collapse} to strengthen it.
...
% % TODO: describe our verification of the fundamental theorem of forcing

% \paragraph{The fundamental theorem of forcing}
% % TODO

% \paragraph{The independence of CH}
After verifying that the (shallow) interpretations of \(\mathsf{CH}\) and \(\neg \mathsf{CH}\) are \(\top\) in \lil{bSet 𝔹_collapse} and \lil{bSet 𝔹_cohen}, we construct deeply-embedded Boolean-valued \lil{L_ZFC}-structures \lil{V 𝔹_collapse} and \lil{V 𝔹_cohen} on top of \lil{bSet 𝔹_collapse} and \lil{bSet 𝔹_cohen}. Completing the fundamental theorem of forcing is merely a matter of checking that the deeply-embedded first-order formulas for the axioms of \(\ZFC\) are correctly interpreted as their shallow counterparts, which were proven separately to be \(\top\). Finally, we verify that the deeply-embedded interpretations of \lil{CH_formula} coincide with the shallow interpretations of \(\mathsf{CH}\).
...
lemma unprovable_of_model_neg {T : Theory L} {f : sentence L} (S : bStructure L 𝔹)
  ...
  {Γ : 𝔹} (H_nonzero : (⊥ : 𝔹) < Γ) (H : Γ ⊩[S] ∼f) : ¬ (T ⊢' f)

lemma V_𝔹_cohen_models_neg_CH : ⊤ ⊩[V 𝔹_cohen] ∼CH_formula

lemma V_𝔹_collapse_models_CH : ⊤ ⊩[V 𝔹_collapse] CH_formula
\end{lstlisting}
...
\begin{lstlisting}
def independent {L : Language} (T : Theory L)
  (f : sentence L) : Prop := ¬ T ⊢' f ∧ ¬ T ⊢' ∼f
...
% TODO(jesse): make this sound less lame
% While the \(\Delta\)-system lemma (\Cref{lemma:delta-system-lemma})
...
for every \(\Gamma : \mathbb{B}\), \(\lambda\; x \; y, \Gamma \leq x =^{\mathbb{B}} y\) is an equivalence relation on \lil{bSet 𝔹} (the larger \(\Gamma\) is, the finer the equivalence relation).
...
Interestingly, we never needed to use transfinite recursion while developing large swathes of elementary set theory in and \lil{pSet} and \lil{bSet 𝔹}. We see the prevalence of transfinite recursion in traditional presentations of set theory as being only a consequence of the traditional definitions of \(V\) and \(V^{\mathbb{B}}\) using transfinite recursion. By instead encoding \(V\) and \(V^{\mathbb{B}}\) as inductive types which expose \(\in\)-induction as their native induction principle, we almost completely eliminate transfinite induction from our formalization.

% talk about L
Our consistency proof of \(\CH\) is very different from the traditional proof, due to G\"odel, which shows that the constructible universe \(\mathsf{L}\) satisfies \(\mathsf{GCH}\). An obvious path to constructing \(\mathsf{L}\) is to define the definable powerset operation with an inductive predicate on \lil{pSet} whose constructors encode the nine G\"odel operations, and then to build the constructible hierarchy by transfinite recursion. It is interesting to consider whether there is a definition of \(\mathsf{L}\) in the same spirit as \lil{pSet} which completely avoids transfinite induction.
...
While our custom automation saved a considerable amount of work, most of it was, in a sense, only a substitute for a more principled approach by \emph{reflection}. The natural deduction and equality reasoning tactics in \Cref{subsect:natded} and \Cref{subsect:bv-cc} make it easier to manually replay a first-order proof of a theorem of \(\mathsf{ZFC}\) in \(\mathbb{B}\), but in principle the Boolean-valued soundness theorem performs this replay automatically given a deeply-embedded first-order proof tree. Ideally, automation would reify a \(\mathbb{B}\)-valued goal to the corresponding first-order statement, discharge it by an ATP, encode the solution in our deeply-embedded natural deduction calculus, then apply soundness. Alternately, one could perform proof transfer via the ordinary completeness theorem. If a goal only requires first-order reasoning in \(\mathsf{ZFC}\), then one can prove it in an arbitrary ordinary model of \(\mathsf{ZFC}\) (where Lean's automation is more effective), use the completeness theorem to turn this into a first-order proof tree, and then replay it using soundness. The advantage to this approach is that a proof would be computed once, then reused in all models, ordinary or \(\mathbb{B}\)-valued, whereas in our formalization, we occasionally had to prove the same statement twice, in \lil{pSet} and \lil{bSet 𝔹}.
