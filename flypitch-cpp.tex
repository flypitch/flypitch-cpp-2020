...
Our formulation of \(\CH\) states that there is no ordinal between \(\omega\) and \(\mathcal{P}(\omega)\). We define an ordinal using the following definitions:
\begin{lstlisting}
eps_transitive z := ∀ x, x ∈ z ⟹ x ⊆ z
eps_trichotomy z := ∀ x y ∈ z, x = y ∨ x ∈ y ∨ y ∈ x
eps_wellfounded z := ∀ x, x ⊆ z ⟹ x ≠ ∅ ⟹
  ∃ y ∈ x, ∀ w ∈ x, w ∉ y
Ord(z) := eps_trichotomy z ∧ eps_wellfounded z ∧
  eps_transitive z
\end{lstlisting}
these are not formulated the same in Lean, where we use de Bruijn variables, and note that \lstinline{epsilon_wellfounded} follows for every set from the axiom of regularity, but we add it for completeness.
