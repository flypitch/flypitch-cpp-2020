...
G\"odel \cite{godel1938consistency} proved in 1938 that $\mathsf{CH}$ was consistent with Zermelo–Fraenkel set theory ($\ZFC$), and later conjectured that $\mathsf{CH}$ is independent of $\ZFC$, i.e. neither provable nor disprovable from the $\ZFC$ axioms.
...
We also use a custom coercion to turn \(\B\)-valued implications into functions.
\begin{lstlisting}
example {a b c : 𝔹} :
  (a ⟹ b) ⊓ (b ⟹ c) ≤ a ⟹ c :=
