...
%   \end{tikzcd}
% \]
We will freely conflate \lil{pSet} ordinals with their underlying types, so e.g. \(\nu : \aleph_2\) means \lil{\nu : \aleph_2.type}. (It is always true that the cardinality of \lil{(ordinal.mk κ).type} is \lil{κ}.) Since in general, \(\widecheck{\aleph_1}\) is \emph{not} what \lil{bSet 𝔹} thinks is \(\aleph_1\), we will use a superscript, e.g. \(\aleph_k^{\mathbb{B}}\), to denote the internal alephs of \lil{bSet 𝔹}.
...
The basic strategy of Cohen forcing is to choose \lil{𝔹} such that for every \lil{ν : ℵ₂.type}, there is a canonical indicator function (a ``Cohen real'') \(\chi_{\nu} : \N \to \mathbb{B}\).
...
This is done by the tactic \lil{bv_imp_intro}:, as in the following example.
\begin{lstlisting}
example {a b c : 𝔹} :
  (a ⟹ b) ⊓ (b ⟹ c) ≤ a ⟹ c :=
