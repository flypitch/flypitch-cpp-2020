...
However, the cost of taking the \lstinline{𝔹}-names at face value is that we must work with them as a \(\mathbb{B}\)-valued model (c.f. \ref{def-bModel}) of \(\mathsf{ZFC}\); equivalently, the calculus of the forcing relation \cite{shoenfield1971unramified}, a key technical tool in usual forcing arguments, is replaced with the calculation of Boolean truth-values in \lstinline{𝔹}.

We always force over a type universe \lil{Type u}, and our Boolean-valued models of \(\mathsf{ZFC}\) are always of the form \lil{bSet 𝔹} for some \lil{𝔹 : Type u}. We have already seen that for every complete Boolean algebra \lil{𝔹}, \lil{bSet 𝔹} is a \lil{𝔹}-valued model of \(\mathsf{ZFC}\). However, concrete choices of \lil{𝔹} can affect the truth-values of particular sentences in \(\mathsf{ZFC}\).
