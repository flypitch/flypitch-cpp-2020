...
We use custom domain-specific tactics and automation throughout our formalization, notably a tactic library for simulating natural deduction proofs inside a complete Boolean algebra \Cref{sect:metaprogramming}). This reveals another advantage of working in a proof assistant: the bookkeeping of Boolean truth-values, sometimes regarded as a tedious aspect of the Boolean-valued approach to forcing, can be automated away.
...
\begin{lemma}\label{lemma:function-reflect}
  For every \(y : \text{\lil{pSet}}\) and \(f : \text{\lil{bSet}}\ \mathbb{B}_{\mathsf{collapse}}\) such that \(\text{\lil{bSet}}\ \mathbb{B}_{\mathsf{collapse}}\) models that \lil{f} is a function from \(\omega\) to \(\widecheck{y}\),
