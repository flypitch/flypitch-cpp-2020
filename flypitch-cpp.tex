...
There is always a map \lil{check : pSet → bSet 𝔹}, $x \mapsto \widecheck{x}$; $\widecheck{x}$ might have different properties than $x$, but \(\Delta_0\) properties (i.e. definable with only bounded quantification) are always preserved. In particular, \lil{bSet 𝔹} thinks $\widecheck{\omega}$ is $\omega$.
