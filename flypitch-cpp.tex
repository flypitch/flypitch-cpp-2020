...
For both parts of the independence of the continuum hypothesis, we use forcing to construct a Boolean-valued model of $\mathsf{ZFC}$, using Cohen forcing for the consistency of \(\neg\CH\) and a \(\sigma\)-closed forcing for the consistency of \(\CH\).

A formalization of Cohen forcing and the consistency of $\neg \mathsf{CH}$ already appeared in \cite{DBLP:conf/itp/HanD19}.
...
This means that, e.g.\ equality is interpreted as a function \lil{eq : M → M → 𝔹} satisfying \lil{𝔹}-valued reflexivity, symmetry, and transitivity (for example, transitivity is formulated as \lil{eq(x, y) ⊓ eq(y, z) ≤ eq(x, z)} for all \lil{x, y, z}).
...
There is always a map \lil{check : pSet → bSet 𝔹}, $x \mapsto \widecheck{x}$; in general, $\widecheck{x}$ might have different properties than $x$, but \(\Delta_0\) properties (i.e. definable with only bounded quantification) are always preserved. In particular, \lil{bSet 𝔹} thinks $\widecheck{\omega}$ is $\omega$.
...
and ultimately reduces to an analysis of how truth-values of \(\forall\)-\(\exists\) statements in \lil{bSet 𝔹} can be reflected \emph{back} to \lil{Type u}, and verifying a combinatorial condition on \lil{𝔹}.
...
We say that \(\B\) is \textbf{\(\sigma\)-closed} if there exists a dense suborder \(\mathbb{P}\) of \(\B\) such that every \(\omega\)-indexed downwards chain \(p_0 \geq p_1 \geq \cdots \geq p_n \geq \cdots\) in \(\mathbb{P}\) has a lower bound \(p_{\omega}\) in \(\mathbb{P}\).
\end{defn}
...
We found that in practice \lil{f} is only needed to give a finite partial specification of a subset of $\aleph_2 \times \mathbb{N}$ (i.e. a finite set \lil{f.ins} which \emph{must} be in the subset, and a finite set \lil{f.out} which \emph{must not} be in the subset).
We chose this representation to make that information immediately accessible.

The Boolean algebra which we use for forcing $\neg\mathsf{CH}$ is
$$\mathbb{B}_{\text{cohen}} := \operatorname{RO}(2^{\aleph_2 \times \mathbb{N}}),$$
where we equip $2^{\aleph_2 \times \mathbb{N}}$ with the usual product space topology.

\begin{defn}
  We define the \textbf{canonical embedding} of the Cohen poset into \(\B_{\text{cohen}}\) as follows:
\begin{lstlisting}
...
\end{defn}
Crucially, the image of this embedding is a dense suborder of \(\mathbb{B}_{\text{cohen}}\).
Recalling that $\leq$ in $\B$ is subset-inclusion, we see that this is essentially because the image of $\iota : \mathbb{P}_{\text{cohen}} \to \B_{\text{cohen}}$ \emph{is} the standard basis for the product topology.
Our chosen encoding of the Cohen poset also made it easier to perform this identification when formalizing this proof.

To show that \(\B_{\text{cohen}}\) has the CCC, we use a powerful combinatorial argument called the \emph{$\Delta$-system lemma}. A family $(A_i)_i$ of sets is called a \textbf{$\Delta$-system} if there is a set $r$, called the \textbf{root} such that whenever $i \ne j$ we have $A_i \cap A_j = r$.
...
\begin{defn}\label{def:cohen-real}
  ...
  We can use \Cref{def-powerset} to get a $\B$-valued subset $\widetilde{\chi_{\nu}} \subseteq \widecheck{\N}$. We call $\widetilde{\chi_{\nu}}$ a \textbf{Cohen real}.
\end{defn}
...
We will then use the CCC in \(\B_{\text{cohen}}\) to show that the reflected \(\forall\)-\(\exists\) statement implies a contradiction.
...
For every \(\eta \in g^{-1}(\{\nu\})\), let \(A_{\eta}\) be the element of \(\B_{\text{cohen}}\) given by the lemma, i.e.
...
\begin{defn}\label{def-collapse-poset}
  ...
  \[D_p := \{g : \aleph_1 \to \mathcal{P}(\omega) \hspace{2mm} | \hspace{2mm} g \text{ extends } p\} , \hspace{3mm} p \in \mathbb{P}_{\text{collapse}}\]
  form the basis of a topology \(\tau\) (finer than the product topology) on the function set \(\mathcal{P}(\omega)^{\aleph_1}\).
  We put
  \[\B_{\text{collapse}} := \operatorname{RO}\left(\mathcal{P}(\omega)^{\aleph_1}, \tau\right).\]
\end{defn}
...
\begin{lemma}\label{lemma-collapse-algebra-sigma-closed}
  ...
  \(\B_{\text{collapse}}\) is \(\sigma\)-closed.
\end{lemma}

\begin{proof}
  We show that the collection of principal open sets \(\{D_p\}_p\) forms a dense subset of
  \(\B_{\text{collapse}}\) such that every $\omega$-indexed downwards chain has a principal open set as lower bound.
  It is clearly dense, because it generates the topology, so suppose we have a downwards chain
  \[D_{p_0} \supseteq D_{p_1} \supseteq \cdots \supseteq D_{p_n} \supseteq \cdots\] in \(\mathbb{P}\).
  This means that we have $p_0 \subseteq p_1 \subseteq \cdots \supseteq p_n \supseteq \cdots$.
  We can now set $p_\omega := \bigcup_i p_i$, which is again a countable partial function, and it is easy to see that $D_{p_\omega}$ is a lower bound of $\{D_{p_i}\}_i$.
\end{proof}
...
Surjectivity is equivalent to \lil{⨅ j, (⨆ i, af i j) = ⊤}, totality is equivalent to \lil{⨅ i, (⨆ j, af i j) = ⊤}, and well-definedness follow from the following two conditions:
\begin{lstlisting}
...
(While the operation \lil{bv_powerset} (\Cref{def-bv-powerset}) gives a construction of the internal powerset of any \lil{x : bSet 𝔹} (using \lil{𝔹}-valued indicator functions, for any \lil{𝔹}), \(\aleph_1^{\mathbb{B}}\) is only specified as the least ordinal greater than \(\omega\), and does not admit as direct a a construction.)
