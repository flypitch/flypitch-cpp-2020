...
Suppose that there is a surjection \(\check{\aleph_1} \twoheadrightarrow \check{\aleph_2}\). Applying this lemma to \(x := \check{\aleph_1}\), \(y := \check{\aleph_2}\), we obtain a \(\forall\)-\(\exists\) statement in the metatheory to which we can apply Lean's axiom of choice to produce a function \(g : \aleph_2 \to \aleph_1\). Since externally, we know that \(\aleph_1 \prec \aleph_2\), it follows from the infinite pigeonhole principle that \(g\) must have an uncountable fiber over some \(\nu < \aleph_1\). For every \(\eta \in g^{-1}(\{\nu\})\), let \(A_{\eta}\) be the element of \lil{𝔹_cohen} given by the lemma, i.e. \[A_{\eta} := \text{\lil{(is_func f) ⊓ (pair (ℵ₁.func ν)̌  (ℵ₂.func η)̌  ∈ᴮ f)}}\]

Because each \(A_{\eta}\) carries around the knowledge that \(f\) is a function as a conjunct, for \(\eta_1 \neq \eta_2\), \(A_{\eta_1}\) and \(A_{\eta_2}\) are incompatible, i.e. \(A_{\eta_1} \sqcap A_{\eta_2} = \bot\). Since the lemma guarantees that each \(A_{\eta}\) is nonzero, the \(A_{\eta}\) form an uncountable antichain. Therefore, if \(\mathbb{B}\) has the CCC, there is a contradiction, so by \ref{lemma-cohen-algebra-CCC}, we have forced \(\neg\mathsf{CH}\) in \lil{bSet 𝔹_cohen}.
...
To specify the surjection \(\check{\aleph_1} \twoheadrightarrow \check{\mathcal{P}(\omega)}\), we specify a subset (the graph of the function) of the powerset \(\mathcal{P}(\check{\aleph_1} \times \check{\mathcal{P}(\omega)})\). In \lil{bSet 𝔹_collapse}, we can do this by specifying the indicator function \(\chi_{\pi}\) of the graph of a function \lil{π : ℵ₁̌  → 𝒫(ω)̌ } as follows: to an \(\eta < \aleph_1\) and a subset \(S \subseteq \mathcal{P}(\omega)\) (in \lil{pSet}), we attach the \emph{principal open} (comprising functions extending the singleton countable partial function \(\{(\eta, S)\}\)):
\[
...
To finish demonstrating that \(\CH\) is true in \lil{bSet 𝔹_collapse}, it remains to check that \(\check{\mathcal{P}(\omega)} = \mathcal{P}(\omega)\) and \(\check{\aleph_1} = \aleph_1\). There are two major obstacles. The first is that to even formally state the latter equality, we must construct \(\aleph_1\) in \lil{bSet 𝔹}. (While the operation \lil{bv_powerset} (\ref{def-bv-powerset}) gives a construction of the internal powerset of any \lil{x : bSet 𝔹} (using \lil{𝔹}-valued indicator functions, for any \lil{𝔹}), \(\aleph_1\) is only specified as the least ordinal greater than \(\omega\), and does not admit as direct a a construction.) We describe our construction of \(\aleph_1\) (as the Hartogs number of \(\omega\)) in \ref{subsection:forcing:aleph-1}.

Now we must ensure that no new countable ordinals are added to \(\aleph_1\) and that no new subset of \(\omega\) are added to \(\mathcal{P}(\omega)\) in the passage from \lil{pSet} to \lil{bSet}. The second obstacle is reducing both statements to, and proving, the following lemma:

\begin{lemma}\label{lemma-function-reflect-suffices}
  Let \(\mathbb{B}\) be a complete Boolean algebra, and suppose that for every \lil{ y : pSet}, and every \lil{f : bSet 𝔹} such that \lil{bSet 𝔹} models that \lil{f} is a function from \(\omega\) to \lil{y̌}, there exists a \lil{g : pSet} such that \lil{g} is a function from \(\omega\) to \lil{y} in \lil{pSet}, and \lil{bSet 𝔹} models that \lil{ǧ = f}. Then \(\check{\mathcal{P}(\omega)} = \mathcal{P}(\omega)\) and \(\check{\aleph_1} = \aleph_1\).
\end{lemma}

We describe our solution to this in \ref{subsect:function-reflection}.
...
\begin{enumerate}
\item We \emph{define} the underlying type \(\alpha\) for \(\aleph_1\) to be \lil{𝒫(ω × ω).type}.
...
\begin{proof}[Proof of \ref{lemma-function-reflect-suffices}.]
  % Observe that by definition of \(\aleph_1\), we have that for every ordinal \(\eta\), \(\eta < \aleph_1\) if and only if there exists a surjection \(\omega \twoheadrightarrow \eta\) (equivalently, there is an injection \(\eta \hookrightarrow \omega\)). Also, \(\mathcal{P}(\omega) \simeq \check{2}^{\check{\omega}}\) (in \(\mathsf{ZF}\)).

  To see that \(\check{\aleph_1} \subseteq \aleph_1\), let \(x\) be an arbitrary element of \(\check{\aleph_1}\). By definition \(x\) is equal to \(\check{\eta}\) for some \(\eta < \aleph_1\) in \lil{pSet}. Since the ordinals and cardinals of \lil{pSet} are isomorphic to Lean's ordinals and cardinals for \lil{Type u}, \(\eta\) injects into \(\omega\) (in \lil{pSet}, and also at the level of indexing types). Since being an injective function is \(\Delta_0\), it is absolute for \lil{check}, so \(x = \check{\eta}\) injects into \(\omega\). Then, by definition of \(\aleph_1\), \(x \in \aleph_1\). \footnote{Note that this did not use our assumption, and holds for general \(\mathbb{B}\). For a conventional proof in a set-theoretic metatheory, see e.g. \cite{bell2011set}}

  To see that \(\aleph_1 \subseteq \check{\aleph_1}\), suppose towards a contradiction that this is not true; since the ordinals are well-ordered, this means that \(\check{\aleph_1} < \aleph_1\), so by definition of \(\aleph_1\), there is a surjection \(f : \omega \to \check{\aleph_1}\). By assumption, this surjection can be lifted to a function \(g : \omega \to \aleph_1\) in \lil{pSet}, which can again be checked to be surjective, a contradiction.

 Similarly, it is true for general \(\mathbb{B}\) that for any \lil{x : pSet}, \(\check{\mathcal{P}(x)} \subseteq \mathcal{P}(\check{x})\), because indicator functions into \lil{bool} naturally induce indicator functions to \(\mathbb{B}\) (by composing with the canonical inclusion \lil{bool → 𝔹}). Conversely, to show that \(\mathcal{P}(\omega) \subseteq \check{\mathcal{P}(\omega)}\), use the isomorphism \(\mathcal{P}(\omega) \simeq \check{2}^{\check{\omega}}\) to reduce this to showing that \(\check{2}^{\check{\omega}} \subseteq \check{2^{\omega}}\), and then apply the assumption to an arbitrary element of \(\check{2}^{\check{\omega}}\).
\end{proof}

It remains to show that \lil{𝔹_collapse} fulfills the assumptions of Lemma \ref{lemma-function-reflect-suffices}.

\begin{lemma}\label{lemma-function-reflect}
  For every \lil{ y : pSet}, and every \lil{f : bSet 𝔹_collapse} such that \lil{bSet 𝔹_collapse} models that \lil{f} is a function from \(\omega\) to \lil{y̌}, there exists a \lil{g : pSet} such that \lil{g} is a function from \(\omega\) to \lil{y} in \lil{pSet}, and \lil{bSet 𝔹_collapse} models that \lil{ǧ = f}.
\end{lemma}
...
\end{lstlisting}
  ...
  It suffices to construct a single function \(g : \omega \to y\) such that \(\bot < D_p \sqcap \check{g} = f\). As with Cohen forcing, we will reflect a Boolean-valued \(\forall\)-\(\exists\) statement into the metatheory, and then use a combinatorial property of \lil{𝔹_collapse} to strengthen it. The following lemma is true for general \(\mathbb{B}\):
...
There are many possible generalizations of our result.
