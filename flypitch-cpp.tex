...
After we verify the (shallow) statements of all the axioms in \lil{bSet 𝔹}, the last step is to prove that the deeply embedded statements correspond to the shallowly embedded statements. This is trivial for the axioms, since it is true by reflexivity, but it takes a little bit of work for the axiom scheme of collection. This proves the following theorem.

\begin{thm}[Fundamental Theorem of Forcing]
  For any complete Boolean algebra $\B$ the model $M = \text{\lil{bSet 𝔹}}$ models $\ZFC$, i.e.\ $\llbracket \ZFC \rrbracket_M=\top$.
\end{thm}
